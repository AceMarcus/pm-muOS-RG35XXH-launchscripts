...
# Check if the resolution is not in 4:3 format
resolution=$(xrandr | grep -oP '\d+x\d+' | head -n 1)

# Convert the resolution to lowercase for comparison
resolution_lower=$(echo "$resolution" | tr '[:upper:]' '[:lower:]')

if [ "$resolution_lower" = "640x480" ]; then
  echo "Resolution is not in 4:3 format."
  HUNT="Hunt.pck"
else
